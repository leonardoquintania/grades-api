...
const update = async (req, res) => {
  ...
  const id = req.params.id;
  try {
    const grade = await gradesModel.findOneAndUpdate({ _id: id }, req.body, {
      new: true,
    });
...
const remove = async (req, res) => {
  const id = req.params.id;
  try {
    const grade = await gradesModel.findOneAndDelete({ _id: id });
...
const removeAll = async (req, res) => {
  try {
    const grade = await gradesModel.deleteMany({});
    res.send({
